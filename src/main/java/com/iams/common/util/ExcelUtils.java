...
/**
 * 
 * @Description: Excel导入导出工具类 remark：参考
...
public class ExcelUtils {
	...
	public static <T> List<T> importExcel(String filePath, Integer titleRows,
		...
		try {
			return ExcelImportUtil.importExcel(new File(filePath), pojoClass,
					params);
	...
	public static <T> List<T> importExcel(MultipartFile file, Integer titleRows,
		...
		try {
			return importExcel(file.getInputStream(), titleRows, headerRows,
					pojoClass);
		} catch (Exception e) {
			throw new IOException(e.getMessage());
		}
